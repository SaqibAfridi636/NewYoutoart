...
const ChatSection = ({ selectedChat, newMessage, setNewMessage, sendMessage }) => {
  ...
  if (!selectedChat) {
    return <div className="flex-1 flex items-center justify-center text-gray-400">Select a chat</div>;
  }
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 bg-gray-100">
        {selectedChat.messages.map((m) => (
          <React.Fragment key={m.id}>
            {m.timeChip && <TimeChip text={m.timeChip} />}
            <div className={`flex ${m.sender === "me" ? "justify-end" : "justify-start"} mb-2`}>
              <div
                className={`max-w-[60%] px-4 py-2 rounded-2xl text-sm shadow-sm
                  ${m.sender === "me" ? "bg-blue-500 text-white rounded-br-md" : "bg-white text-gray-800 rounded-bl-md"}`}
              >
                {m.text}
              </div>
            </div>
